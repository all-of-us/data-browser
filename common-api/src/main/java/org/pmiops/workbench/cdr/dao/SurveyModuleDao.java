...
public interface SurveyModuleDao extends CrudRepository<SurveyModule, Long> {
  ...
  @Query(nativeQuery=true, value= "select m.name, m.description, m.concept_id, COUNT(DISTINCT a.concept_id) as question_count, m.participant_count, m.order_number, m.can_show from \n" +
          ...
          "union distinct \n" +
          "select distinct c.* from question_concept c where concept_id in (?3) and match(question_string) against(?1 in boolean mode) > 0\n" +
          "union distinct\n" +
          "select distinct c.* from question_concept c join \n" +
          "achilles_results ar1 on c.concept_id=ar1.stratum_2\n" +
          "join question_concept c2 on ar1.stratum_3=c2.concept_id\n" +
          "where ar1.stratum_2 in (?3) and ar1.analysis_id=3110\n" +
          "and match(c2.question_string) against(?1 in boolean mode) > 0\n" +
          "union distinct \n" +
          "select distinct c.* from question_concept c where concept_id in (?2) and match(concept_name) against(?1 in boolean mode) > 0\n" +
          "union distinct\n" +
          "select distinct c.* from question_concept c join \n" +
          "achilles_results ar1 on c.concept_id=ar1.stratum_2\n" +
          "join question_concept c2 on ar1.stratum_3=c2.concept_id\n" +
