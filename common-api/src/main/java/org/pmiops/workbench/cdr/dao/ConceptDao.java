...
public interface ConceptDao extends CrudRepository<Concept, Long> {
    ...
    @Query(value = "select distinct c1.* from concept_relationship cr " +
            ...
            "order by c.name asc) " +
            "and c1.concept_class_id = 'Ingredient') and (c1.count_value > 0 or c1.source_count_value > 0)", nativeQuery = true)
    ...
    @Query(value = "select distinct c1.* from concept_relationship cr " +
            ...
            "and match(c.name, c.code) against(?1 in boolean mode) > 0 " +
            "order by c.name asc) " +
            "and c1.concept_class_id = 'Ingredient') and cr.concept_id_2 not in (?2) and (c1.count_value > 0 or c1.source_count_value > 0)", nativeQuery = true)
