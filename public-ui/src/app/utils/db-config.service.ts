...
export class DbConfigService {
  ...
  PM_CONCEPTS = [
    903118, 903115, 903133, 903121, 903135, 903136, 903126, 903111, 903120,
  ];
  FITBIT_MEASUREMENTS = [
    "Any Fitbit Data",
    "Heart Rate (Summary)",
  ...
  conceptIdNames = [
    ...
    { conceptId: 1740639, conceptName: "Personal and Family Health History" },
  ...
  constructor(private api: DataBrowserService) {
    ...
    group.concepts.push(
      new ConceptWithAnalysis("903118", "Systolic", chartType)
    );
    group.concepts.push(
      new ConceptWithAnalysis("903115", "Diastolic", chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("height", "Height");
    group.concepts.push(
      new ConceptWithAnalysis("903133", group.groupName, chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("weight", "Weight");
    group.concepts.push(
      new ConceptWithAnalysis("903121", group.groupName, chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("mean-waist", "Mean waist circumference");
    group.concepts.push(
      new ConceptWithAnalysis("903135", group.groupName, chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("mean-hip", "Mean hip circumference");
    group.concepts.push(
      new ConceptWithAnalysis("903136", group.groupName, chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("mean-heart-rate", "Mean heart rate");
    group.concepts.push(
      new ConceptWithAnalysis("903126", group.groupName, chartType)
    );
    this.pmGroups.push(group);
    ...
    group.concepts.push(
      new ConceptWithAnalysis("903111", group.groupName, chartType)
    );
    this.pmGroups.push(group);

    group = new ConceptGroup("pregnancy", "Pregnancy");
    group.concepts.push(
      new ConceptWithAnalysis("903120", group.groupName, chartType)
    );
    this.pmGroups.push(group);
  }
  ...
  public matchPhysicalMeasurements(searchString: string) {
    ...
    return this.pmGroups.filter((conceptgroup) =>
      conceptgroup.groupName.toLowerCase().includes(searchString.toLowerCase())
    ).length;
  }

  public matchFitbitMeasurements(searchString: string) {
    return this.FITBIT_MEASUREMENTS.filter((name) =>
