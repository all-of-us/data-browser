...
export const HeatMapReactComponent = class extends React.Component<
  Props,
  State
> {
  territories: string[];
  ...
  options = {
    ...
    tooltip: {
      ...
      formatter: function () {
        const value =
          this.point.value <= 20 ? "≤ 20" : this.point.value.toLocaleString();
        ...
        const percentageDisplay =
          percentage < 0.05 ? "< 0.1%" : `${percentage.toFixed(1)}%`;
          ...
          events: {
            mouseOver: function () {
              const matchedKey = Object.keys(
                HeatMapReactComponent.keyGroups
              ).find((group) =>
                HeatMapReactComponent.keyGroups[group].includes(this["hc-key"])
              );
              if (matchedKey) {
                ...
                this.update(
                  {
                    ...
                    color: getHoverColor(
                      this.series.chart.userOptions.hoverColor
                    ),
                  },
                  false
                );
            ...
            mouseOut: function () {
              ...
              if (matchedKey) {
                HeatMapReactComponent.getOriginalColor(
                  this.series,
                  HeatMapReactComponent.keyGroups[matchedKey]
                );
              } else {
                this.update(
                  {
    ...
    series: [
      {
        data: this.formatLocationData(
          this.props.locationAnalysis.results,
          this.props.domain
        ),
        borderColor: "#ffffff",
        borderWidth: 0.1,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.color !== this.props.color) {
      ...
      this.options.series[0].states.hover.color = getHoverColor(
        this.props.color
      );
    }
    ...
    if (
      prevProps.selectedResult !== this.props.selectedResult &&
      this.props.domain === "genomic"
    ) {
      // Update the data
      const newData = this.formatLocationData(
        this.props.locationAnalysis.results,
        this.props.domain
      );
  ...
  applyTerritoryColors() {
    const chartObj = this.chartRef.current?.chart;
    if (!chartObj || !chartObj.series || !chartObj.series[0]) {
      return;
    }
      ...
      hcKeys.forEach((hcKey) => {
        ...
        foundPaths.forEach((pathEl) => {
          ...
          pathEl.addEventListener("mouseenter", () =>
            this.handleMouseEnterGroup(groupName)
          );
          pathEl.addEventListener("mouseleave", () =>
            this.handleMouseLeaveGroup(groupName)
          );
        });
      });
    });
  ...
  handleMouseEnterGroup(groupName) {
    ...
    pathsInGroup.forEach((pathEl) => {
      if (!(pathEl as any).dataset.originalColor) {
        (pathEl as any).dataset.originalColor =
          (pathEl as HTMLElement).style.fill ||
          getComputedStyle(pathEl as HTMLElement).fill;
      }
  ...
  formatLocationData(data, domain) {
    ...
    const keyGroups = {
      "us-vi": {
        keys: HeatMapReactComponent.keyGroups.VI_KEYS,
        name: "U.S. Virgin Islands",
      },
      "us-mp": {
        keys: HeatMapReactComponent.keyGroups.NMI_KEYS,
        name: "Northern Mariana Islands",
      },
      "us-as": {
        keys: HeatMapReactComponent.keyGroups.AS_KEYS,
        name: "American Samoa",
      },
      "us-pr": {
        keys: HeatMapReactComponent.keyGroups.PR_KEYS,
        name: "Puerto Rico",
      },
    ...
    if (domain === "genomic" && this.props.selectedResult) {
      data = data.filter((r) => r.stratum4 === this.props.selectedResult);
    }
