...
export class ValueReactChartComponent extends React.Component<Props, State> {
  ...
  getStackedChartOptions() {
    const {
      valueAnalysis: { analysisName, results },
    } = this.props;
  ...
  getValueChartOptions() {
    const {
      valueAnalysis: { analysisName, results, unitName },
      genderId,
      chartTitle,
    } = this.props;
    const valueResults = !results ? [] : results;
    const { categories, series } = this.prepValueCatsAndData(
      valueResults.filter((r) => r.stratum3 === genderId),
      analysisName
    );
    this.setValueChartOptions(
      analysisName,
      categories,
      series,
      chartTitle,
...
export class ValueChartWrapperComponent extends BaseReactWrapper {
  ...
  constructor() {
    super(ValueReactChartComponent, [
      "conceptId",
      "valueAnalysis",
      "genderId",
      "chartTitle",
