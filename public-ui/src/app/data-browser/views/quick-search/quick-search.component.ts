...
export class QuickSearchComponent implements OnInit, OnDestroy {
  ...
  public viewSurvey(r, search: string) {
    ...
    this.dbc.conceptIdNames.forEach(idName => {
      if (r.conceptId === idName.conceptId) {
        if (search) {
          this.router.navigate(['survey/' + idName.conceptName.toLowerCase().split(' ').join('-') + '/' + search]);
        } else {
 ...
 getPMUrl() {
    return this.searchText.value ? 'physical-measurements/' + this.searchText.value : 'physical-measurements';
 }
}
