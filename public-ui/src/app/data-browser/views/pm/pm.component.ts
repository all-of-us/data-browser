...
export class PhysicalMeasurementsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiConceptAnalysisResults();


    this.loadingStack.push(true);
    this.apiCountAnalysis();
  }

  public apiCountAnalysis() {
    return this.api.getCountAnalysis('Physical Measurements', 'pm')
      .then(
        result => {
  ...
  public apiConceptAnalysisResults() {
    return this.api.getConceptAnalysisResults(this.dbc.PM_CONCEPTS.map(String))
      .then(
        result => {
