...
function import_wait () {
  ...
  seconds_waited=5
  while true; do
    if [[ $(gcloud sql operations list --instance $INSTANCE --project $PROJECT | grep ".*RUNNING") ]] || [[ $(gcloud sql operations list --instance $INSTANCE --project $PROJECT | grep ".*PENDING") ]]
    then
        sleep $wait_interval
