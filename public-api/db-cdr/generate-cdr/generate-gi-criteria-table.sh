...
"CREATE OR REPLACE TABLE \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`
(
  id            INT64,
  parent_id     INT64,
  type          STRING,
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (id,parent_id,type,subtype,code,name,is_group,is_selectable,est_count,domain_id,concept_id,has_attribute,path,stratum_1)
...
left join
  (select concept_id, gender_identity, count(distinct person) cnt from
    ...
    (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
    WHERE a.OBSERVATION_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
    UNION DISTINCT
    SELECT a.person_id as person, ob.value_source_concept_id as gender_identity, concept_id
    FROM \`$BQ_PROJECT.$BQ_DATASET.measurement\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
    (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
    WHERE a.MEASUREMENT_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
    UNION DISTINCT
    SELECT a.person_id as person, ob.value_source_concept_id as gender_identity, concept_id
    FROM \`$BQ_PROJECT.$BQ_DATASET.procedure_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
    (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
    WHERE a.PROCEDURE_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838) x
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` x
SET x.est_count = y.cnt
from (select c.id, gender_identity, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD9' and is_group = 1 and is_selectable = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
    from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` a,
    (
      SELECT a.person_id as person, ob.value_source_concept_id as gender_identity, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
      UNION DISTINCT
      SELECT a.person_id as person, ob.value_source_concept_id as gender_identity, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
      ...
      FROM \`$BQ_PROJECT.$BQ_DATASET.procedure_occurrence\` a,\`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
      WHERE a.PROCEDURE_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
    ) b
...
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD10' and subtype = 'CM' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
    from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` a,
    (
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD10' and subtype = 'CM' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
      UNION DISTINCT
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
...
SET x.est_count = y.cnt
from (select c.id, gi, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD10' and subtype = 'PCS' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
  from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` a,
    (
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a,\`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'ICD10' and subtype = 'PCS' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
      UNION DISTINCT
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
...
left join
  (select concept_id, gi, count(distinct person) cnt from
    (SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
	  ...
	  WHERE a.drug_SOURCE_CONCEPT_ID = b.concept_Id and a.person_id = ob.person_id and ob.observation_source_concept_id=1585838
	  UNION DISTINCT
	  SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
	  FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
	  (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
	  WHERE a.OBSERVATION_SOURCE_CONCEPT_ID = b.concept_id and a.person_id = ob.person_id and ob.observation_source_concept_id=1585838
	  UNION DISTINCT
	  SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
	  FROM \`$BQ_PROJECT.$BQ_DATASET.measurement\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
	  (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
	  WHERE a.MEASUREMENT_SOURCE_CONCEPT_ID = b.concept_id and a.person_id = ob.person_id and ob.observation_source_concept_id=1585838
	  UNION DISTINCT
	  SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
	  FROM \`$BQ_PROJECT.$BQ_DATASET.procedure_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
	  (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.criteria_seed\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
	  WHERE a.PROCEDURE_SOURCE_CONCEPT_ID = b.concept_id and a.person_id = ob.person_id and ob.observation_source_concept_id=1585838) x
  GROUP BY 1,2) c on b.concept_id = c.concept_id
where type = 'CPT'
and (is_group = 1 or (is_group = 0 and is_selectable = 1 and (c.cnt != 0 or c.cnt is not null)))
order by 1"
...
SET x.est_count = y.cnt
from (select c.id, gi, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'CPT' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
  from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` a,
    (
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.drug_exposure\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
      WHERE a.drug_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
      UNION DISTINCT
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
      WHERE a.OBSERVATION_SOURCE_CONCEPT_ID = b.concept_id and ob.person_id = a.person_id and ob.observation_source_concept_id=1585838
      UNION DISTINCT
      SELECT a.person_id as person, ob.value_source_concept_id as gi, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.measurement\` a, \`$BQ_PROJECT.$BQ_DATASET.observation\` ob,
...
or (is_group = 1 and id not in (select parent_id from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`
...
do
   ...
   bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
   "insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, stratum_1)
   select (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`)+1 AS ID, 0, 'SNOMED', 'CM', CONCEPT_CODE, CONCEPT_NAME, 1, 0, 'Condition', concept_id, 0, cast($t as string)
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`), t.ID, 'SNOMED', 'CM',
b.CONCEPT_CODE, b.CONCEPT_NAME, 1,0, 'Condition', b.CONCEPT_ID, 0, CAST(t.id as STRING) as path, t.stratum_1
  from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` t
	   join \`$BQ_PROJECT.$BQ_DATASET.snomed_rel_cm_in_data\` b on t.code = b.p_concept_code
 where (id) not in (select PARENT_ID from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`)
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
    select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`),
    ...
    where (id) not in (select PARENT_ID from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`)
    and t.stratum_1 in ('1585839','903070','1585840','1585842','1585841','1585843','903079','903096')"
done

echo "SNOMED - CM - add parents as children in achilles criteria with gender stratum"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
select (row_number() over (order by PARENT_ID, NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`)) as ID, *
from
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute,stratum_1)
    select (select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`)+1, 0, 'SNOMED', 'PCS', CONCEPT_CODE, CONCEPT_NAME, 1, 0, 'Procedure', concept_id, 0, cast($t as string)
    from \`$BQ_PROJECT.$BQ_DATASET.concept\`
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path,stratum_1)
select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`), t.ID, 'SNOMED', 'PCS',
...
from
  ...
  where p.observation_source_concept_id=1585838
  group by 1,2) y
...
do
   ...
   bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
   "insert into \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`
     (id,parent_id,type,subtype,code,name,is_group,is_selectable,domain_id,concept_id,has_attribute,path,stratum_1)
   select ROW_NUMBER() OVER(order by concept_name) + (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`) as ID,
     0,'SNOMED','MEAS',concept_code,concept_name,1,0,'Measurement',concept_id,1,
     CAST(ROW_NUMBER() OVER(order by concept_name) + (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`) as STRING) as path, cast($t as string)
   from
...
select row_number() over (order by t.id, b.concept_name)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`),
  ...
  CONCAT(t.path, '.',
    CAST(row_number() over (order by t.id, b.concept_name)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`) AS STRING)),t.stratum_1
from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` t
...
id as parent_id,type,subtype,code,name,cnt,0,1,domain_id,concept_id,1,CONCAT(path, '.',
  CAST(row_number() over (order by PARENT_ID, NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\`) as STRING)),cast(gender as string)
from
  (select *
  from \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` a
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.gi_criteria\` set est_count = -1 where est_count is null"
