...
"CREATE OR REPLACE TABLE \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`
(
  id            INT64,
  parent_id     INT64,
  type          STRING,
...
# ICD9
################################################
echo "ICD9 - inserting data (do not insert zero count children) into achilles criteria with  age stratum for rolled up counts"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (id,parent_id,type,subtype,code,name,is_group,is_selectable,est_count,domain_id,concept_id,has_attribute,path,stratum_1)
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` x
SET x.est_count = y.cnt
from (select c.id, age, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD9' and is_group = 1 and is_selectable = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
    from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` a,
    (
      SELECT a.person_id as person, 2 as age, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.person\` p,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD9' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and p.person_id = a.person_id
      and (extract(year from condition_start_date) - p.year_of_birth) >= 18 and (extract(year from condition_start_date) - p.year_of_birth) < 30
      UNION DISTINCT
      SELECT a.person_id as person, 2 as age, concept_id
...
# ICD10
################################################
echo "ICD10 - CM - insert data (do not insert zero count children) in achilles criteria with age decile stratum"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (id,parent_id,type,subtype,code,name,is_group,is_selectable,est_count,domain_id,concept_id,has_attribute,path,stratum_1)
select ID, PARENT_ID, "TYPE", SUBTYPE, CODE, NAME, is_group, is_selectable,
case when is_group = 0 and is_selectable = 1 then c.cnt else null end as est_count,
...
echo "ICD10 - PCS - insert data (do not insert zero count children) in achilles criteria with age stratum"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (id,parent_id,type,subtype,code,name,is_group,is_selectable,est_count,domain_id,concept_id,has_attribute,path,stratum_1)
select ID, PARENT_ID, "TYPE", SUBTYPE, CODE, NAME, is_group, is_selectable,
case when is_group = 0 and is_selectable = 1 then c.cnt else null end as est_count,
...
echo "ICD10 - CM - generate parent counts in achilles criteria with age stratum"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` x
SET x.est_count = y.cnt
from (select c.id, age, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD10' and subtype = 'CM' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
    from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` a,
    (
      SELECT a.person_id as person, 2 as age, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.person\` p,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD10' and subtype = 'CM' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and p.person_id = a.person_id
      and (extract(year from condition_start_date) - p.year_of_birth) >= 18 and (extract(year from condition_start_date) - p.year_of_birth) < 30
      UNION DISTINCT
      SELECT a.person_id as person, 2 as age, concept_id
...
echo "ICD10 - PCS - generate parent counts in achilles criteria with age stratum"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` x
SET x.est_count = y.cnt
from (select c.id, age, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD10' and subtype = 'PCS' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
  from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` a,
    (
      SELECT a.person_id as person, 2 as age, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.condition_occurrence\` a, \`$BQ_PROJECT.$BQ_DATASET.person\` p,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'ICD10' and subtype = 'PCS' and is_group = 0 and is_selectable = 1) b
      WHERE a.CONDITION_SOURCE_CONCEPT_ID = b.concept_id and p.person_id = a.person_id
      and (extract(year from condition_start_date) - p.year_of_birth) >= 18 and (extract(year from condition_start_date) - p.year_of_birth) < 30
      UNION DISTINCT
      SELECT a.person_id as person, 2 as age, concept_id
...
SET x.est_count = y.cnt
from (select c.id, age, count(distinct person) cnt
from (select * from (select id from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'CPT' and parent_id != 0 and is_group = 1) a
left join \`$BQ_PROJECT.$BQ_DATASET.criteria_ancestor_count\` b on a.id = b.ancestor_id) c
left join
  (select a.id, b.*
  from \`$BQ_PROJECT.$BQ_DATASET.criteria\` a,
    (
      SELECT a.person_id as person, '2' as age, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.drug_exposure\` a, \`$BQ_PROJECT.$BQ_DATASET.person\` p,
      (select concept_id, path from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
      WHERE a.drug_SOURCE_CONCEPT_ID = b.concept_id and a.person_id=p.person_id
      and (extract(year from drug_exposure_start_date) - p.year_of_birth) >= 18 and (extract(year from drug_exposure_start_date) - p.year_of_birth) < 30
      UNION DISTINCT
      SELECT a.person_id as person, '2' as age, concept_id
      FROM \`$BQ_PROJECT.$BQ_DATASET.observation\` a, \`$BQ_PROJECT.$BQ_DATASET.person\` p,
      (select concept_id, path, code from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` where type = 'CPT' and is_group = 0 and is_selectable = 1) b
      WHERE a.OBSERVATION_SOURCE_CONCEPT_ID = b.concept_id and a.person_id=p.person_id
      and (extract(year from observation_date) - p.year_of_birth) >= 18 and (extract(year from observation_date) - p.year_of_birth) < 30
      UNION DISTINCT
      SELECT a.person_id as person, '2' as age, concept_id
...
or (is_group = 1 and id not in (select parent_id from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, stratum_1)
    select (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`)+1 AS ID, 0, 'SNOMED', 'CM', CONCEPT_CODE, CONCEPT_NAME, 1, 0, 'Condition', concept_id, 0, cast($i as string)
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`), t.ID, 'SNOMED', 'CM',
b.CONCEPT_CODE, b.CONCEPT_NAME, 1,0, 'Condition', b.CONCEPT_ID, 0, CAST(t.id as STRING) as path, t.stratum_1
  from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` t
	   join \`$BQ_PROJECT.$BQ_DATASET.snomed_rel_cm_in_data\` b on t.code = b.p_concept_code
 where (id) not in (select PARENT_ID from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`)
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
    select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`),
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path, stratum_1)
select (row_number() over (order by PARENT_ID, NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`)) as ID, *
from
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute,stratum_1)
    select (select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`)+1, 0, 'SNOMED', 'PCS', CONCEPT_CODE, CONCEPT_NAME, 1, 0, 'Procedure', concept_id, 0, cast($i as string)
    from \`$BQ_PROJECT.$BQ_DATASET.concept\`
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` (ID, PARENT_ID, TYPE, SUBTYPE, CODE, NAME, IS_GROUP, IS_SELECTABLE, domain_id, CONCEPT_ID, has_attribute, path,stratum_1)
select row_number() over (order by t.ID, b.CONCEPT_NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`), t.ID, 'SNOMED', 'PCS',
...
set x.est_count = y.cnt
from
  (SELECT procedure_concept_id as concept_id, floor((extract(year from procedure_date) - p.year_of_birth)/10) as age, count(distinct a.person_id) cnt
  FROM \`$BQ_PROJECT.$BQ_DATASET.procedure_occurrence\` a join \`$BQ_PROJECT.$BQ_DATASET.person\` p on p.person_id = a.person_id
...
do
    ...
    bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
    "insert into \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`
      (id,parent_id,type,subtype,code,name,is_group,is_selectable,domain_id,concept_id,has_attribute,path,stratum_1)
    select ROW_NUMBER() OVER(order by concept_name) + (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`) as ID,
      0,'SNOMED','MEAS',concept_code,concept_name,1,0,'Measurement',concept_id,1,
      CAST(ROW_NUMBER() OVER(order by concept_name) + (SELECT MAX(ID) FROM \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`) as STRING) as path, cast($i as string)
    from
...
select row_number() over (order by t.id, b.concept_name)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`),
  ...
  CONCAT(t.path, '.',
    CAST(row_number() over (order by t.id, b.concept_name)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`) AS STRING)),t.stratum_1
from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` t
...
id as parent_id,type,subtype,code,name,cnt,0,1,domain_id,concept_id,1,CONCAT(path, '.',
  CAST(row_number() over (order by PARENT_ID, NAME)+(select max(id) from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\`) as STRING)),cast(age as string)
from
  (select *
  from \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` a
...
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"UPDATE \`$BQ_PROJECT.$BQ_DATASET.age_criteria\` set est_count = -1 where est_count is null"
