...
datasets=$(bq --project_id=$PUBLIC_PROJECT ls --max_results=500)
if [ -z "$datasets" ]
then
...
tables=$(bq --project_id=$PUBLIC_PROJECT --dataset_id=$PUBLIC_DATASET ls --max_results=100)
c_re=\\bconcept\\b
...
where r.type='SNOMED' and r.domain_id='CONDITION' and r.full_text like '%condition_rank1%' and is_standard = 1
...
from  (select r.concept_id, est_count as count from \`$PUBLIC_PROJECT.$PUBLIC_DATASET.cb_criteria\` r
where r.type='SNOMED' and r.domain_id='CONDITION' and r.full_text like '%condition_rank1%' and is_standard = 0
group by r.concept_id, count) as r
where r.concept_id = c.concept_id and c.domain_id='Condition' and c.vocabulary_id='SNOMED' "
...
set c.count_value = r.count
from  (select r.concept_id, est_count as count from \`$PUBLIC_PROJECT.$PUBLIC_DATASET.cb_criteria\` r
where r.type='SNOMED' and r.domain_id='PROCEDURE' and r.full_text like '%procedure_rank1%' and is_standard=1
group by r.concept_id, count) as r
...
set c.source_count_value = r.count
from  (select r.concept_id, est_count as count from \`$PUBLIC_PROJECT.$PUBLIC_DATASET.cb_criteria\` r
where r.type='SNOMED' and r.domain_id='PROCEDURE' and r.full_text like '%procedure_rank1%' and is_standard=0
group by r.concept_id, count) as r
where r.concept_id = c.concept_id and c.domain_id='Procedure' and c.vocabulary_id='SNOMED' "
...
from  (select r.concept_id, est_count as count from \`$PUBLIC_PROJECT.$PUBLIC_DATASET.cb_criteria\` r
where r.type='ICD9CM' and r.domain_id='CONDITION' and r.full_text like '%condition_rank1%'
group by r.concept_id, count) as r
...
set c.count_value = r.count, c.source_count_value = r.count
from  (select r.concept_id, est_count as count from \`$PUBLIC_PROJECT.$PUBLIC_DATASET.cb_criteria\` r
where r.type='ICD10CM' and r.domain_id='CONDITION' and r.full_text like '%condition_rank1%'
group by r.concept_id, count) as r
