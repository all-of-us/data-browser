...
CREATE USER IF NOT EXISTS '${META_DB_USER}'@'%' IDENTIFIED WITH mysql_native_password BY '${META_DB_PASSWORD}';
CREATE USER IF NOT EXISTS '${LIQUIBASE_DB_USER}'@'%' IDENTIFIED WITH mysql_native_password BY '${LIQUIBASE_DB_PASSWORD}';
CREATE USER IF NOT EXISTS '${PUBLIC_DB_USER}'@'%' IDENTIFIED WITH mysql_native_password BY '${PUBLIC_DB_PASSWORD}';

-- In case the users already exist, change their passwords.
SET PASSWORD FOR '${META_DB_USER}'@'%' = '${META_DB_PASSWORD}';
SET PASSWORD FOR '${LIQUIBASE_DB_USER}'@'%' = '${LIQUIBASE_DB_PASSWORD}';
SET PASSWORD FOR '${PUBLIC_DB_USER}'@'%' = '${PUBLIC_DB_PASSWORD}';

GRANT SELECT, INSERT, UPDATE, DELETE, CREATE TEMPORARY TABLES ON ${DB_NAME}.* TO '${META_DB_USER}'@'%';
