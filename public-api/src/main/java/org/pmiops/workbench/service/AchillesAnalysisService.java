...
public class AchillesAnalysisService {
    ...
    public Analysis getGenderAnalysis() {
        Analysis genderAnalysis = achillesMapper.dbModelToClient(achillesAnalysisDao.findAnalysisById(GENDER_ANALYSIS));
    ...
    public List<ConceptAnalysis> getFitbitConceptAnalyses(List<String> concepts) {
        List<Analysis> analysisList = achillesAnalysisDao.findAnalysisByIdsAndDomain(ImmutableList.of(GENDER_ANALYSIS_ID, AGE_ANALYSIS_ID, COUNT_ANALYSIS_ID, PARTICIPANT_COUNT_BY_DATE_ANALYSIS_ID), "Fitbit").stream()
                .map(achillesMapper::dbModelToClient)
                .collect(Collectors.toList());
    ...
    public List<ConceptAnalysis> getConceptAnalyses(List<String> conceptIds, String domainId) {
        ...
        List<DbAchillesResultDist> overallDistResults = achillesResultDistService.fetchByAnalysisIdsAndConceptIds(MEASUREMENT_DIST_ANALYSIS_ID, conceptIds);
        ...
        for(String conceptId: conceptIds){
            ...
            List<Analysis> analysisList = achillesAnalysisDao.findConceptAnalysisResults(conceptId,ImmutableList.of(GENDER_ANALYSIS_ID, AGE_ANALYSIS_ID, COUNT_ANALYSIS_ID,
                    MEASUREMENT_GENDER_ANALYSIS_ID, MEASUREMENT_DIST_ANALYSIS_ID, MEASUREMENT_GENDER_UNIT_ANALYSIS_ID)).stream()
                    .map(achillesMapper::dbModelToClient)
                    .collect(Collectors.toList());
                        ...
                        for (String unit : conceptDistResultsByUnit.keySet()) {
                            if (results.keySet().contains(unit)) {
                                ...
                                if (!unit.equalsIgnoreCase("no unit")) {
                                    processMeasurementGenderMissingBins(MEASUREMENT_DIST_ANALYSIS_ID, unitGenderAnalysis, conceptId, unit, new ArrayList<>(conceptDistResultsByUnit.get(unit)), "numeric");
                                } else {
                                    ...
                                    if (textValues.size() > 0) {
                                        processMeasurementGenderMissingBins(MEASUREMENT_DIST_ANALYSIS_ID, unitGenderAnalysis, conceptId, null, null, "text");
                                    }
                                    if (numericValues.size() > 0) {
                                        processMeasurementGenderMissingBins(MEASUREMENT_DIST_ANALYSIS_ID, unitGenderAnalysis, conceptId, null, null, "numeric");
                                    }
                                    unitGenderAnalysis.setResults(results.get(unit));
                                    unitGenderAnalysis.setUnitName(unit);
            ...
            if (isMeasurement) {
                ...
                Analysis measurementDistAnalysis = achillesMapper.dbModelToClient(achillesAnalysisDao.findAnalysisById(MEASUREMENT_DIST_ANALYSIS_ID));
