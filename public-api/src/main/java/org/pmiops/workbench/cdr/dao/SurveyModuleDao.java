...
public interface SurveyModuleDao extends CrudRepository<DbSurveyModule, Long> {
  ...
  @Query(nativeQuery=true, value= "select m.name, m.description, m.concept_id, COUNT(DISTINCT a.concept_id) as question_count, m.participant_count, m.order_number from \n" +
          ...
          "union distinct \n" +
          "select distinct c.* from survey_metadata c where concept_id in (?3) and (match(question_string) against(?1 in boolean mode) > 0 or match(concept_name) against(?1 in boolean mode) > 0)\n" +
          "union distinct\n" +
          ...
          "union distinct \n" +
          "select distinct c.* from survey_metadata c where concept_id in (?2) and (match(concept_name) against(?1 in boolean mode) > 0 or match(concept_name) against(?1 in boolean mode) > 0)\n" +
          "union distinct\n" +
          "select distinct c.* from survey_metadata c join \n" +
          "achilles_results ar1 on c.concept_id=ar1.stratum_2\n" +
          "join achilles_results ar2 on ar1.stratum_3=ar2.stratum_2\n" +
