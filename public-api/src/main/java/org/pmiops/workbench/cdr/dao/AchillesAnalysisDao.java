...
public interface AchillesAnalysisDao extends CrudRepository<AchillesAnalysis, Long> {
    ...
    @Query(value = "select distinct a from AchillesAnalysis a left join FETCH a.results as r " +
            "where r.stratum1 = ?1 and a.analysisId in (3110,3111,3112,3113,3114,3320,3321) order by a.analysisId"
    )
    List<AchillesAnalysis> findSurveyAnalysisResults(String survey_concept_id);
