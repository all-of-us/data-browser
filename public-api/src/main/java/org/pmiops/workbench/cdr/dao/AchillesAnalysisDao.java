...
public interface AchillesAnalysisDao extends CrudRepository<AchillesAnalysis, Long> {
    ...
    @Query(value = "select distinct a from AchillesAnalysis a left join FETCH a.results as r " +
            "where r.stratum1 = ?1 and r.stratum2 in (?2) and a.analysisId in (3110,3111,3112,3113,3114) order by a.analysisId"
    )
    ...
    @Query(value = "select distinct a from AchillesAnalysis a left join FETCH a.results as r where a.analysisId=?1")
