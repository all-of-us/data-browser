...
public interface AchillesAnalysisDao extends CrudRepository<DbAchillesAnalysis, Long> {
    ...
    @Query(value = "select distinct a from DbAchillesAnalysis a left join FETCH a.results r where a.analysisId in (?1) and r.stratum3 = ?2")
    ...
    @Query(value = "select distinct a from DbAchillesAnalysis a left join FETCH a.results r where a.analysisId in (?1) and r.stratum1 = ?2")
