...
public interface CBCriteriaDao extends CrudRepository<DbCriteria, Long> {
    ...
    @Query(value = "select c.id as id, c.parent_id as parent_id, c.domain_id as domain_id, c.is_standard as is_standard, c.type as type, c.subtype as subtype, " +
            ...
            " from cb_criteria c join concept co on c.concept_id=co.concept_id where c.concept_id = :conceptId and match(c.synonyms) against(:matchWord in boolean mode)\n" +
            "union all\n" +
            ...
            " from cb_criteria c join concept co on c.concept_id=co.concept_id where parent_id in (select distinct id from cb_criteria where concept_id=:conceptId and match(synonyms) against(:matchWord in boolean mode))\n" +
