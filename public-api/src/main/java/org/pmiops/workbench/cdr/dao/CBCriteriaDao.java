...
public interface CBCriteriaDao extends CrudRepository<DbCriteria, Long> {
    ...
    @Query(value = "select c.id as id, c.parent_id as parent_id, c.domain_id as domain_id, c.is_standard as is_standard, c.type as type, c.subtype as subtype, " +
            ...
            " from cb_criteria c join concept co on c.concept_id=co.concept_id where c.concept_id = :conceptId and match(c.full_text) against(:matchWord in boolean mode)\n" +
            "union all\n" +
            ...
            " from cb_criteria c join concept co on c.concept_id=co.concept_id where parent_id in (select distinct id from cb_criteria where concept_id=:conceptId and match(full_text) against(:matchWord in boolean mode))\n" +
            "order by est_count desc", nativeQuery = true)
    List<DbCriteria> findParentCounts(@Param("conceptId") String conceptId, @Param("matchWord") String matchWord);
