...
public interface QuestionConceptDao extends CrudRepository<DbQuestionConcept, Long> {
    @Query(nativeQuery=true, value="select * from question_concept where survey_concept_id=?1 and generate_counts=1 and sub=0")
    List<DbQuestionConcept> getSurveyQuestions(Long survey_concept_id);
    ...
    List<DbQuestionConcept> getMatchingSurveyQuestions(Long survey_concept_id, String search_word);

    @Query(nativeQuery=true, value="select distinct * from question_concept c where concept_id in (?1)")
    List<DbQuestionConcept> getFMHQuestions(List<String> questionConceptIds);
    ...
    List<DbQuestionConcept> getMatchingFMHQuestions(List<String> questionConceptIds, String search_word);
    ...
    List<DbQuestionConcept> getSubQuestionsLevel1(String questionConceptId, String answerConceptId, String surveyConceptId);
    ...
    List<DbQuestionConcept> getSubQuestionsLevel2(String answerConceptId);
}
