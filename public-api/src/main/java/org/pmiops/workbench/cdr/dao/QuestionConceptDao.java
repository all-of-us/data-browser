...
public interface QuestionConceptDao extends CrudRepository<QuestionConcept, Long> {
    ...
    @Query(nativeQuery=true, value="select distinct c.concept_id,c.concept_name,c.domain_id,c.synonyms,c.concept_code,c.count_value,c.prevalence, 0 as match_type from question_concept c join achilles_results \n" +
    ...
    @Query(nativeQuery=true, value="select distinct c.concept_id, c.concept_name, c.domain_id, c.synonyms, c.concept_code, c.count_value, c.prevalence, 1 as match_type from question_concept c where concept_id in (?1) and match(concept_name) against(?2 in boolean mode) > 0\n" +
            "union distinct\n" +
            ...
            "union distinct\n" +
            "select distinct c.concept_id, c.concept_name, c.domain_id, c.synonyms, c.concept_code, c.count_value, c.prevalence, 0 as natch_type from question_concept c join \n" +
