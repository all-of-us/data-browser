...
public class GenomicsController implements GenomicsApiDelegate {
    ...
    private static final String genomicRegionRegex =
            "(?i)([\"]*)(chr([0-9]{1,})*[XYxy]*:{0,}).*";
    private static final String variantIdRegex =
            "(?i)([\"]*)((\\d{1,}|X|Y)-\\d{5,}-[A,C,T,G]{1,}-[A,C,T,G]{1,}).*";
    private static final String rsNumberRegex = "(?i)(rs)(\\d{1,})";

    private static final String svVariantIdRegexV7 =
            "(?i)AoUSVPhase[a-zA-Z0-9]{1,2}\\.chr[1-9XY][0-9]?(?:\\.final_cleanup_)?"
                    + "(BND|DUP|DEL|INS|CPX|INV|CTX|CNV)_chr[1-9XY][0-9]?_\\d+";
    private static final String svVariantIdRegexV8 =
            "(?i)AoUSVPhase[a-zA-Z0-9]{1,2}\\.(BND|DUP|DEL|INS|CPX|INV|CTX|CNV)"
                    + "_chr[1-9XY][0-9]?_shard[0-9][0-9]?_\\d+";
    private static final String svVariantIdRegexV9 =
            "(?i)AoU_srWGS_SV\\.v\\d+\\.(BND|DUP|DEL|INS|CPX|INV|CTX|CNV)"
                    + "_chr[1-9XY][0-9]?_shard[0-9][0-9]?_\\d+";
    private static final String svVariantIdRegexRandom =
            "(?i)(\\d{1,2}|X|Y)-\\d{1,10}-[0-9a-fA-F]{2}";
    private static final String svVariantIdRegexRefined =
            "(?i)(\\d{1,2}|X|Y)-(\\d{1,10})([a-z])?";

    // ====================================================================
    // WHERE-CLAUSE FRAGMENTS (appended by getSearchType / getSVSearchType)
    // ====================================================================
    ...
    private static final String WHERE_VARIANT_ID_OR_VCF =
            " where variant_id = @variant_id OR variant_id_vcf = @variant_id";
    private static final String WHERE_RS_NUMBER_CONTAINS =
            ", unnest(rs_number) AS rsid\n where REGEXP_CONTAINS(rsid, @rs_id)";
    ...
    private static final String VARIANT_DETAIL_SQL_TEMPLATE =
            "SELECT dna_change, transcript, ARRAY_TO_STRING(rs_number, ', ') as rs_number, "
                    + "gvs_afr_ac as afr_allele_count, gvs_afr_an as afr_allele_number, "
                    + "gvs_afr_af as afr_allele_frequency, gvs_afr_hc as afr_homozygote_count, "
                    + "gvs_eas_ac as eas_allele_count, gvs_eas_an as eas_allele_number, "
                    + "gvs_eas_af as eas_allele_frequency, gvs_eas_hc as eas_homozygote_count, "
                    + "gvs_eur_ac as eur_allele_count, gvs_eur_an as eur_allele_number, "
                    + "gvs_eur_af as eur_allele_frequency, gvs_eur_hc as eur_homozygote_count, "
                    + "gvs_amr_ac as amr_allele_count, gvs_amr_an as amr_allele_number, "
                    + "gvs_amr_af as amr_allele_frequency, gvs_amr_hc as amr_homozygote_count, "
                    + "gvs_mid_ac as mid_allele_count, gvs_mid_an as mid_allele_number, "
                    + "gvs_mid_af as mid_allele_frequency, gvs_mid_hc as mid_homozygote_count, "
                    + "gvs_sas_ac as sas_allele_count, gvs_sas_an as sas_allele_number, "
                    + "gvs_sas_af as sas_allele_frequency, gvs_sas_hc as sas_homozygote_count, "
                    + "gvs_oth_ac as oth_allele_count, gvs_oth_an as oth_allele_number, "
                    + "gvs_oth_af as oth_allele_frequency, gvs_oth_hc as oth_homozygote_count, "
    ...
    // and reused across all 7 UNION ALL branches.
    private static final String FILTER_OPTION_SQL_PREFIX =
            "WITH filtered AS (\n"
    ...
    private static final String FILTER_OPTION_SQL_SUFFIX =
            ")\n"
                    + "SELECT 'Gene' AS option, gene AS genes, '' AS conseq, '' AS variant_type,\n"
                    + "       '' AS clin_significance,\n"
                    + "       COUNT(*) AS gene_count, 0 AS con_count, 0 AS variant_type_count,\n"
                    + "       0 AS clin_count, 0 AS min_count, 0 AS max_count\n"
                    + "FROM filtered, UNNEST(SPLIT(genes, ', ')) gene\n"
                    + "GROUP BY gene\n"
                    + "UNION ALL\n"
                    + "SELECT 'Consequence', '', con, '', '', 0, COUNT(*), 0, 0, 0, 0\n"
                    + "FROM filtered LEFT JOIN UNNEST(consequence) AS con\n"
                    + "GROUP BY con\n"
                    + "UNION ALL\n"
                    + "SELECT 'Variant Type', '', '', variant_type, '', 0, 0, COUNT(*), 0, 0, 0\n"
                    + "FROM filtered GROUP BY variant_type\n"
                    + "UNION ALL\n"
                    + "SELECT 'Clinical Significance', '', '', '', clin, 0, 0, 0, COUNT(*), 0, 0\n"
                    + "FROM filtered LEFT JOIN UNNEST(clinical_significance) AS clin\n"
                    + "GROUP BY clin\n"
                    + "UNION ALL\n"
                    + "SELECT 'Allele Count', '', '', '', '', 0, 0, 0, 0,\n"
                    + "       MIN(allele_count), MAX(allele_count) FROM filtered\n"
                    + "UNION ALL\n"
                    + "SELECT 'Allele Number', '', '', '', '', 0, 0, 0, 0,\n"
                    + "       MIN(allele_number), MAX(allele_number) FROM filtered\n"
                    + "UNION ALL\n"
                    + "SELECT 'Homozygote Count', '', '', '', '', 0, 0, 0, 0,\n"
                    + "       MIN(homozygote_count), MAX(homozygote_count) FROM filtered;";

    // ====================================================================
    // SV (aou_sv_vcf_9_processed) SQL TEMPLATES
    // ====================================================================
    ...
    private static final String SV_VARIANT_LIST_SQL_TEMPLATE =
            "SELECT variant_id, variant_type, consequence, position, a.size, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN a.cn_nonref_count "
                    + "ELSE a.allele_count END AS allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN a.cn_number "
                    + "ELSE a.allele_number END AS allele_number, \n"
    ...
    private static final String SV_VARIANT_DETAIL_SQL_TEMPLATE =
            "SELECT variant_type, consequence_genes, position, size, variant_id_vcf, \n"
                    + "cpx_intervals as cpx_intervals, cpx_type as cpx_type, "
                    + "a.filter as filter, a.no_call_rate as no_call_rate, "
                    + "a.quality_score as quality_score, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_afr "
                    + "ELSE afr_ac END as afr_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_afr "
                    + "ELSE afr_an END as afr_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_afr "
                    + "ELSE afr_af END as afr_allele_frequency, \n"
                    + "afr_n_homalt as afr_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_eas "
                    + "ELSE eas_ac END as eas_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_eas "
                    + "ELSE eas_an END as eas_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_eas "
                    + "ELSE eas_af END as eas_allele_frequency, \n"
                    + "eas_n_homalt as eas_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_eur "
                    + "ELSE eur_ac END as eur_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_eur "
                    + "ELSE eur_an END as eur_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_eur "
                    + "ELSE eur_af END as eur_allele_frequency, \n"
                    + "eur_n_homalt as eur_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_amr "
                    + "ELSE amr_ac END as amr_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_amr "
                    + "ELSE amr_an END as amr_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_amr "
                    + "ELSE amr_af END as amr_allele_frequency, \n"
                    + "amr_n_homalt as amr_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_mid "
                    + "ELSE mid_ac END as mid_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_mid "
                    + "ELSE mid_an END as mid_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_mid "
                    + "ELSE mid_af END as mid_allele_frequency, \n"
                    + "mid_n_homalt as mid_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_sas "
                    + "ELSE sas_ac END as sas_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_sas "
                    + "ELSE sas_an END as sas_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_sas "
                    + "ELSE sas_af END as sas_allele_frequency, \n"
                    + "sas_n_homalt as sas_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count_oth "
                    + "ELSE oth_ac END as oth_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number_oth "
                    + "ELSE oth_an END as oth_allele_number, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_freq_oth "
                    + "ELSE oth_af END as oth_allele_frequency, \n"
                    + "oth_n_homalt as oth_homozygote_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count "
                    + "ELSE allele_count END as total_allele_count, \n"
                    + "CASE WHEN variant_type = '<CNV>' THEN cn_number "
                    + "ELSE allele_number END as total_allele_number, \n"
    ...
    private static final String SV_FILTER_OPTION_SQL_SUFFIX =
            ")\n"
                    ...
                    + "FROM filtered, UNNEST(SPLIT(consequence, ', ')) con\n"
                    + "GROUP BY con\n"
                    + "UNION ALL\n"
                    + "SELECT 'Size', '', '', '', '', MIN(size), MAX(size) FROM filtered\n"
                    + "UNION ALL\n"
                    + "SELECT 'Allele Count', '', '', '', '',\n"
                    + "       MIN(CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count\n"
                    + "                ELSE allele_count END),\n"
                    + "       MAX(CASE WHEN variant_type = '<CNV>' THEN cn_nonref_count\n"
                    + "                ELSE allele_count END)\n"
                    + "FROM filtered\n"
                    + "UNION ALL\n"
                    + "SELECT 'Allele Number', '', '', '', '',\n"
                    + "       MIN(CASE WHEN variant_type = '<CNV>' THEN cn_number\n"
                    + "                ELSE allele_number END),\n"
                    + "       MAX(CASE WHEN variant_type = '<CNV>' THEN cn_number\n"
                    + "                ELSE allele_number END)\n"
                    + "FROM filtered\n"
                    + "UNION ALL\n"
    ...
    public GenomicsController(AchillesAnalysisService achillesAnalysisService,
    ...
    public ResponseEntity<Analysis> getParticipantCounts() {
        ...
        return ResponseEntity.ok(
                achillesAnalysisService.findAnalysisByIdAndDomain(3000L, "Genomics"));
    }

    @Override
    public ResponseEntity<AnalysisListResponse> getChartData() {
        initCdrVersion();
        ImmutableList<Long> analysisIds = ImmutableList.of(
                CommonStorageEnums.analysisIdFromName(AnalysisIdConstant.GENO_GENDER_ANALYSIS),
        ...
        analysisListResponse.setItems(
                achillesAnalysisService.findAnalysisByIdsAndDomain(analysisIds, "Genomics"));
    ...
    @Override
    public ResponseEntity<Long> getVariantSearchResultSize(
            VariantResultSizeRequest variantResultSizeRequest) {
        initCdrVersion();
    ...
    @Override
    public ResponseEntity<VariantListResponse> searchVariants(
        ...
        if (sortMetadata != null) {
            ...
            if (geneColumnSortMetadata != null && geneColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY genes "
                        + getSortDirection(geneColumnSortMetadata.getSortDirection(), false);
            }

            SortColumnDetails consequenceColumnSortMetadata = sortMetadata.getConsequence();
            if (consequenceColumnSortMetadata != null
                    && consequenceColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = buildConsequenceSeverityOrderBy(
                        consequenceColumnSortMetadata.getSortDirection().equals("asc"));
            }

            SortColumnDetails variantTypeColumnSortMetadata = sortMetadata.getVariantType();
            if (variantTypeColumnSortMetadata != null
                    && variantTypeColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY lower(variant_type) "
                        + getSortDirection(variantTypeColumnSortMetadata.getSortDirection(), false);
            }
            ...
            if (clinSigColumnSortMetadata != null && clinSigColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY (SELECT STRING_AGG(distinct d, \", \" "
                        + "order by d asc) FROM UNNEST(clinical_significance) d) "
            ...
            if (alleleCountColumnSortMetadata != null
                    && alleleCountColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY allele_count "
                        + getSortDirection(alleleCountColumnSortMetadata.getSortDirection(), false);
            }

            SortColumnDetails alleleNumberColumnSortMetadata = sortMetadata.getAlleleNumber();
            if (alleleNumberColumnSortMetadata != null
                    && alleleNumberColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY allele_number "
                        + getSortDirection(alleleNumberColumnSortMetadata.getSortDirection(), false);
            }
            ...
            if (alleleFrequencyColumnSortMetadata != null
                    && alleleFrequencyColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY allele_frequency "
                        + getSortDirection(alleleFrequencyColumnSortMetadata.getSortDirection(), false);
            }

            SortColumnDetails homozygoteCountColumnSortMetadata = sortMetadata.getHomozygoteCount();
            if (homozygoteCountColumnSortMetadata != null
                    && homozygoteCountColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY homozygote_count "
                        + getSortDirection(homozygoteCountColumnSortMetadata.getSortDirection(), false);
            }
        }
        ...
        finalSql += " LIMIT " + rowCount
                + " OFFSET " + ((Optional.ofNullable(page).orElse(1) - 1) * rowCount);
    ...
    public ResponseEntity<VariantInfo> getVariantDetails(String variant_id) {
        ...
        VariantInfo variantInfo = new VariantInfo()
                ...
                .totalAlleleFrequency(
                        bigQueryService.getDouble(row, rm.get("total_allele_frequency")))
                .totalHomozygoteCount(
                        bigQueryService.getLong(row, rm.get("total_homozygote_count")));
    ...
    public ResponseEntity<GenomicFilters> getGenomicFilterOptions(String variantSearchTerm) {
        ...
        // Single-scan filter options query (replaces 7-scan CTE structure).
        String finalSql = FILTER_OPTION_SQL_PREFIX + searchSqlQuery + FILTER_OPTION_SQL_SUFFIX;
    ...
    @Override
    public ResponseEntity<Long> getSVVariantSearchResultSize(
            SVVariantResultSizeRequest variantResultSizeRequest) {
        initCdrVersion();
    ...
    @Override
    public ResponseEntity<SVVariantListResponse> searchSVVariants(
            SearchSVVariantsRequest searchVariantsRequest) {
        initCdrVersion();
        ...
        if (sortMetadata != null) {
            ...
            if (variantTypeColumnSortMetadata != null
                    && variantTypeColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY variant_type "
                        + getSortDirection(variantTypeColumnSortMetadata.getSortDirection(), true);
            }

            SortColumnDetails consequenceColumnSortMetadata = sortMetadata.getConsequence();
            if (consequenceColumnSortMetadata != null
                    && consequenceColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = buildSVConsequenceSeverityOrderBy(
                        consequenceColumnSortMetadata.getSortDirection().equals("asc"));
            }
            ...
            if (sizeColumnSortMetadata != null && sizeColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY size "
                        + getSortDirection(sizeColumnSortMetadata.getSortDirection(), true);
            }

            SortColumnDetails alleleCountColumnSortMetadata = sortMetadata.getAlleleCount();
            if (alleleCountColumnSortMetadata != null
                    && alleleCountColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY CASE WHEN variant_type = '<CNV>' "
                        + "THEN cn_nonref_count ELSE allele_count END "
                        + getSortDirection(alleleCountColumnSortMetadata.getSortDirection(), false);
            }
            ...
            if (alleleNumberColumnSortMetadata != null
                    && alleleNumberColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY CASE WHEN variant_type = '<CNV>' "
                        + "THEN cn_number ELSE allele_number END "
                        + getSortDirection(alleleNumberColumnSortMetadata.getSortDirection(), false);
            }
            ...
            if (alleleFrequencyColumnSortMetadata != null
                    && alleleFrequencyColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY CASE WHEN variant_type = '<CNV>' "
                        + "THEN cn_nonref_freq ELSE allele_frequency END "
                        + getSortDirection(alleleFrequencyColumnSortMetadata.getSortDirection(), false);
            }
            ...
            if (filterColumnSortMetadata != null && filterColumnSortMetadata.isSortActive()) {
                ORDER_BY_CLAUSE = " ORDER BY filter "
                        + getSortDirection(filterColumnSortMetadata.getSortDirection(), true);
            }
        }
        ...
        String finalSql = "SELECT variant_id, variant_type, " + consequenceSelect
                + ", position, a.size, "
                + "CASE WHEN variant_type = '<CNV>' THEN a.cn_nonref_count "
                + "ELSE a.allele_count END AS allele_count, "
                + "CASE WHEN variant_type = '<CNV>' THEN a.cn_number "
    ...
    public ResponseEntity<SVGenomicFilters> getSVGenomicFilterOptions(String variantSearchTerm) {
        ...
        // Single-scan SV filter options query (replaces 8-scan CTE structure).
        String finalSql = SV_FILTER_OPTION_SQL_PREFIX + searchSqlQuery + SV_FILTER_OPTION_SQL_SUFFIX;
    ...
    // ====================================================================
    // SEARCH TERM PARSING (public static — used by endpoints above)
    // ====================================================================
    ...
    public static GenomicSearchTermType getSearchType(String variantSearchTerm, String searchTerm) {
        ...
        if (searchTerm.matches(genomicRegionRegex)) {
            ...
            if (searchTerm.contains(":")) {
                ...
                contig = regionTermSplit[0].substring(0, 3).toLowerCase()
                        + regionTermSplit[0].substring(3).toUpperCase();
            }
    ...
    public static SVGenomicSearchTermType getSVSearchType(
            String variantSearchTerm, String searchTerm) {
        ...
        if (searchTerm.matches(svVariantIdRegexV7)
                || searchTerm.matches(svVariantIdRegexV8)
                || searchTerm.matches(svVariantIdRegexV9)
                || searchTerm.matches(svVariantIdRegexRandom)
                || searchTerm.matches(svVariantIdRegexRefined)) {
            // All SV variant ID patterns search on both variant_id and variant_id_vcf
            variant_id = searchTerm;
            whereVariantIdFlag = true;
    ...
    private static String buildConsequenceSeverityOrderBy(boolean ascending) {
        ...
        for (Map.Entry<String, Integer> entry : CONSEQUENCE_SEVERITY_RANKS.entrySet()) {
            caseStatement.append("WHEN TRIM(d) = '").append(entry.getKey())
        ...
        return " ORDER BY (SELECT MAX(CASE " + caseStatement.toString()
                + "ELSE 0 END) FROM UNNEST(consequence) d) " + direction;
    }
    ...
    private static String buildSVConsequenceSeverityOrderBy(boolean ascending) {
        ...
        for (Map.Entry<String, Integer> entry : SV_CONSEQUENCE_SEVERITY_RANKS.entrySet()) {
            caseStatement.append("WHEN TRIM(con) = '").append(entry.getKey())
                    .append("' THEN ").append(entry.getValue()).append(" ");
        }
        String direction = ascending ? "ASC" : "DESC";
        return " ORDER BY (SELECT MAX(CASE " + caseStatement.toString()
