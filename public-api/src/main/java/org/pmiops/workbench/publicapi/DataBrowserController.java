...
public class DataBrowserController implements DataBrowserApiDelegate {
    ...
    public ResponseEntity<CountAnalysis> getCountAnalysis(String domainId, String domainDesc) {
        ...
        if (domainDesc.equals("ehr") || domainDesc.equals("fitbit")) {
            ...
            List<Analysis> ehrAnalysesList = achillesAnalysisService.findSurveyAnalysisByIds(ImmutableList.of(SURVEY_GENDER_COUNT_ANALYSIS_ID, SURVEY_AGE_COUNT_ANALYSIIS_ID), domainId);
            CountAnalysis surveyCountAnalysis = new CountAnalysis();
            surveyCountAnalysis.setDomainId(domainId);
            Analysis genderCountAnalysis = ehrAnalysesList.stream().filter(aa -> aa.getAnalysisId() == SURVEY_GENDER_COUNT_ANALYSIS_ID).collect(Collectors.toList()).get(0);
            Analysis ageCountAnalysis = ehrAnalysesList.stream().filter(aa -> aa.getAnalysisId() == SURVEY_AGE_COUNT_ANALYSIIS_ID).collect(Collectors.toList()).get(0);
            addGenderStratum(genderCountAnalysis,2, domainId, null);
            addAgeStratum(ageCountAnalysis, domainId, null,  2);
            surveyCountAnalysis.setGenderCountAnalysis(genderCountAnalysis);
            surveyCountAnalysis.setAgeCountAnalysis(ageCountAnalysis);
