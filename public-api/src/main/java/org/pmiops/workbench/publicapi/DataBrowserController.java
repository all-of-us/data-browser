...
public class DataBrowserController implements DataBrowserApiDelegate {
     ...
     * client representation (generated by Swagger).
     */
    private static final Function<DbAchillesAnalysis, org.pmiops.workbench.model.Analysis>
            TO_CLIENT_ANALYSIS =
            new Function<DbAchillesAnalysis, org.pmiops.workbench.model.Analysis>() {
                @Override
                public org.pmiops.workbench.model.Analysis apply(DbAchillesAnalysis cdr) {
                    ...
                    return new org.pmiops.workbench.model.Analysis()
                            ...
                            .results(new ArrayList<AchillesResult>())
    ...
    public ResponseEntity<org.pmiops.workbench.model.Analysis> getGenderAnalysis(){
        ...
        Analysis genderAnalysis = achillesAnalysisService.findAnalysisById(GENDER_ANALYSIS);
    ...
    public ResponseEntity<SurveyQuestionFetchResponse> getSubQuestions(Long surveyConceptId, Long questionConceptId, Long answerConceptId, Integer level) {
        ...
        List<DbAchillesAnalysis> surveyAnalysisList = achillesAnalysisDao.findSubQuestionResults(ImmutableList.of(3110L, 3111L, 3112L, 3113L), questionIds);
    ...
    public ResponseEntity<QuestionConceptListResponse> getFMHSurveyQuestionResults(String questionConceptId, String answerConceptId) {
        ...
        List<DbAchillesAnalysis> analyses = achillesAnalysisDao.findSurveyAnalysisResults("43528698", questionConceptIds);
    ...
    public ResponseEntity<AnalysisListResponse> getSurveyQuestionResults(Long surveyConceptId, Long questionConceptId, String questionPath) {
        ...
        List<DbAchillesAnalysis> surveyAnalysisList = achillesAnalysisDao.findSurveyQuestionResults(ImmutableList.of(3110L, 3111L, 3112L, 3113L), String.valueOf(surveyConceptId), String.valueOf(questionConceptId), questionPath);
    ...
    public ResponseEntity<SurveyVersionCountResponse> getSurveyVersionCounts(Long surveyConceptId) {
        ...
        List<DbAchillesAnalysis> surveyAnalysisList = achillesAnalysisDao.findAnalysisByIds(ImmutableList.of(SURVEY_VERSION_PARTICIPANT_COUNT_ANALYSIS_ID, SURVEY_VERSION_QUESTION_COUNT_ANALYSIS_ID));
        AnalysisListResponse analysisListResponse = new AnalysisListResponse();
        analysisListResponse.setItems(surveyAnalysisList.stream().map(TO_CLIENT_ANALYSIS).collect(Collectors.toList()));
    ...
    public ResponseEntity<CountAnalysis> getCountAnalysis(String domainId, String domainDesc) {
        ...
        if (domainDesc.equals("ehr") || domainDesc.equals("fitbit")) {
            ...
            DbAchillesAnalysis genderCountAnalysis = ehrAnalysesList.stream().filter(aa -> aa.getAnalysisId() == SURVEY_GENDER_COUNT_ANALYSIS_ID).collect(Collectors.toList()).get(0);
            DbAchillesAnalysis ageCountAnalysis = ehrAnalysesList.stream().filter(aa -> aa.getAnalysisId() == SURVEY_AGE_COUNT_ANALYSIIS_ID).collect(Collectors.toList()).get(0);
    ...
    public ResponseEntity<AnalysisListResponse> getSurveyQuestionCounts(String questionConceptId, String questionPath) {
        ...
        List<DbAchillesAnalysis> surveyQuestionCountList = achillesAnalysisDao.findSurveyQuestionCounts(ImmutableList.of(3320L, 3321L), questionConceptId, questionPath);
    ...
    public ResponseEntity<ConceptAnalysisListResponse> getConceptAnalysisResults(List<String> conceptIds, String domainId){
        ...
        for(String conceptId: conceptIds){
            ...
            List<DbAchillesAnalysis> analysisList = achillesAnalysisDao.findConceptAnalysisResults(conceptId,ImmutableList.of(GENDER_ANALYSIS_ID, AGE_ANALYSIS_ID, COUNT_ANALYSIS_ID, MEASUREMENT_GENDER_ANALYSIS_ID, MEASUREMENT_DIST_ANALYSIS_ID, MEASUREMENT_GENDER_UNIT_ANALYSIS_ID));
            ...
            while(it.hasNext()) {
                ...
                DbAchillesAnalysis aa = (DbAchillesAnalysis)pair.getValue();
                ...
                if (analysisId == COUNT_ANALYSIS_ID) {
                    ...
                    Map<String,List<DbAchillesResult>> results = seperateUnitResults(aa);
                    List<DbAchillesAnalysis> unitSeperateAnalysis = new ArrayList<>();
                        ...
                        if(conceptDistResults != null){
                            ...
                            for(String unit: unitDistResults.keySet()){
                                if (results.keySet().contains(unit)) {
                                    DbAchillesAnalysis unitGenderAnalysis = new DbAchillesAnalysis(aa);
                                    ...
                                    if(!unit.equalsIgnoreCase("no unit")) {
                                        ...
                                        ArrayList<DbAchillesResult> textValues = new ArrayList<>();
                                        ArrayList<DbAchillesResult> numericValues = new ArrayList<>();
                                        // In case no unit has a mix of text and numeric values, only display text values as mix does not make sense to user.
                                        for (DbAchillesResult result: unitGenderAnalysis.getResults()) {
                    ...
                    for(String unit: results.keySet()){
                        DbAchillesAnalysis unitGenderCountAnalysis = new DbAchillesAnalysis(aa);
            ...
            if(isMeasurement){
                DbAchillesAnalysis measurementDistAnalysis = achillesAnalysisDao.findAnalysisById(MEASUREMENT_DIST_ANALYSIS_ID);
                ...
                List<DbAchillesAnalysis> unitSeperateAnalysis = new ArrayList<>();
                for(String unit: results.keySet()){
                    DbAchillesAnalysis mDistAnalysis = new DbAchillesAnalysis(measurementDistAnalysis);
    ...
    public void addGenderStratum(Analysis aa, int stratum, String conceptId, List<DbAchillesResult> ehrCountResults){
        ...
        for(DbAchillesResult ar: aa.getResults()){
        ...
        if(uniqueGenderStratums.size() < 3) {
            ...
            for(String missingGender: completeGenderStratumList){
                DbAchillesResult missingResult = null;
                if (aa.getAnalysisId() == EHR_GENDER_COUNT_ANALYSIS_ID) {
                    missingResult = new DbAchillesResult(aa.getAnalysisId(), domainConceptId, null, conceptId, missingGender, null, null, 20L, 20L);
                } else {
                    if (stratum == 1) {
                        missingResult = new DbAchillesResult(aa.getAnalysisId(), missingGender, null, null, null, null, null, 20L, 20L);
                    } else if (stratum == 2) {
                        missingResult = new DbAchillesResult(aa.getAnalysisId(), conceptId, missingGender, null, null, null, null, 20L, 20L);
                    } else if (stratum == 3) {
                        missingResult = new DbAchillesResult(aa.getAnalysisId(), conceptId, null, missingGender, null, null, null, 20L, 20L);
                    }
                }
    ...
    public void addAgeStratum(DbAchillesAnalysis aa, String conceptId, List<DbAchillesResult> ehrCountResults, int stratum){
            ...
            if (aa.getAnalysisId() == EHR_AGE_COUNT_ANALYSIS_ID) {
                ...
                for(String missingAgeDecile: completeAgeDeciles){
                    ...
                    if (ehrAgeCountResults != null && ehrAgeCountResults.size() > 0) {
                        ...
                        missingResult = new DbAchillesResult(EHR_AGE_COUNT_ANALYSIS_ID, conceptId, null, null, missingAgeDecile, null, null, 20L, 20L);
                    } else {
                        missingResult = new DbAchillesResult(EHR_AGE_COUNT_ANALYSIS_ID, conceptId, null, null, missingAgeDecile, null, null, 20L, 20L);
                    }
                ...
                completeAgeDeciles.removeAll(uniqueAgeDeciles);
                for(String missingAgeDecile: completeAgeDeciles){
                    DbAchillesResult missingResult = new DbAchillesResult(AGE_ANALYSIS_ID, conceptId, missingAgeDecile, null, null, null, null, 20L, 20L);
                    missingResult.setAnalysisStratumName(ageStratumNameMap.get(missingAgeDecile));
                    aa.getResults().add(missingResult);
                }
            }
    ...
    public void processMeasurementGenderMissingBins(Long analysisId, DbAchillesAnalysis aa, String conceptId, String unitName, List<DbAchillesResultDist> resultDists, String type) {
        ...
        if (resultDists != null) {
            ...
            for(String maleRemaining: maleRangesInResults){
                ...
                DbAchillesResult achillesResult = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(MALE), maleRemaining, null, String.valueOf(maleBinWidth), 20L, 20L);
            ...
            for(String femaleRemaining: femaleRangesInResults){
                ...
                missingBinWidth = trimTrailingZeroDecimals(missingBinWidth);
                DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(FEMALE), femaleRemaining, null, String.valueOf(femaleBinWidth), 20L, 20L);
                aa.addResult(ar);
            }
            ...
            for(String otherRemaining: otherRangesInResults){
                ...
                missingBinWidth = trimTrailingZeroDecimals(missingBinWidth);
                DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(OTHER), otherRemaining, null, String.valueOf(otherBinWidth), 20L, 20L);
                aa.addResult(ar);
            }
        } else {

            List<DbAchillesResult> maleResults = new ArrayList<>();
            List<DbAchillesResult> femaleResults = new ArrayList<>();
            List<DbAchillesResult> otherResults = new ArrayList<>();
            ...
            if (("numeric").equals(type)) {
                if (maleResults.size() == 0) {
                    DbAchillesResult achillesResult = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, "No Unit", String.valueOf(MALE), "0", null, "0", 20L, 20L);
                    aa.addResult(achillesResult);
                }
                if (femaleResults.size() == 0) {
                    DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(FEMALE), "0", null, "0", 20L, 20L);
                    aa.addResult(ar);
                }
                if (otherResults.size() == 0) {
                    DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(OTHER), "0", null, "0", 20L, 20L);
                    aa.addResult(ar);
                }
            } else if(("text").equals(type)) {
                if (maleResults.size() == 0) {
                    DbAchillesResult achillesResult = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, "No Unit", String.valueOf(MALE), "Null", null, "0", 20L, 20L);
                    aa.addResult(achillesResult);
                }
                if (femaleResults.size() == 0) {
                    DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(FEMALE), "Null", null, "0", 20L, 20L);
                    aa.addResult(ar);
                }
                if (otherResults.size() == 0) {
                    DbAchillesResult ar = new DbAchillesResult(MEASUREMENT_GENDER_ANALYSIS_ID, conceptId, unitName, String.valueOf(OTHER), "Null", null, "0", 20L, 20L);
                    aa.addResult(ar);
                }
            }
    ...
    public static HashMap<String,List<DbAchillesResult>> seperateUnitResults(DbAchillesAnalysis aa){
        ...
        Multimap<String, DbAchillesResult> resultsWithUnits = Multimaps
                .index(aa.getResults(), DbAchillesResult::getStratum2);

        HashMap<String,List<DbAchillesResult>> seperatedResults = new HashMap<>();
    ...
    public List<QuestionConcept> mapAnalysesToQuestions(List<DbAchillesAnalysis> analyses, List<QuestionConcept> questions) {
        Map<Long, List<DbAchillesResult>> countAnalysisResultsByQuestion = new HashMap<>();
        Map<Long, List<DbAchillesResult>> genderAnalysisResultsByQuestion = new HashMap<>();
        Map<Long, List<DbAchillesResult>> ageAnalysisResultsByQuestion = new HashMap<>();
        Map<Long, List<DbAchillesResult>> versionAnalysisResultsByQuestion = new HashMap<>();
            ...
            if (aa.getAnalysisId() == SURVEY_COUNT_ANALYSIS_ID) {
                ...
                for(DbAchillesResult ar: aa.getResults()) {
                    ...
                    if (countAnalysisResultsByQuestion.containsKey(questionId)) {
                        List<DbAchillesResult> tempResults = countAnalysisResultsByQuestion.get(questionId);
                        tempResults.add(ar);
                    } else {
                        List<DbAchillesResult> tempResults = new ArrayList<>();
                        tempResults.add(ar);
            ...
            if (aa.getAnalysisId() == SURVEY_GENDER_ANALYSIS_ID) {
                ...
                for(DbAchillesResult ar: aa.getResults()) {
                    ...
                    if (genderAnalysisResultsByQuestion.containsKey(questionId)) {
                        List<DbAchillesResult> tempResults = genderAnalysisResultsByQuestion.get(questionId);
                        tempResults.add(ar);
                    } else {
                        List<DbAchillesResult> tempResults = new ArrayList<>();
                        tempResults.add(ar);
                ...
                for (DbAchillesResult ar : aa.getResults()) {
                    ...
                    if (validAgeDeciles.contains(ar.getStratum5())) {
                        if (ageAnalysisResultsByQuestion.containsKey(questionId)) {
                            List<DbAchillesResult> tempResults = ageAnalysisResultsByQuestion.get(questionId);
                            tempResults.add(ar);
                        } else {
                            List<DbAchillesResult> tempResults = new ArrayList<>();
                            tempResults.add(ar);
            ...
            if (aa.getAnalysisId() == SURVEY_VERSION_ANALYSIS_ID) {
                ...
                for(DbAchillesResult ar: aa.getResults()) {
                    ...
                    if (versionAnalysisResultsByQuestion.containsKey(questionId)) {
                        List<DbAchillesResult> tempResults = versionAnalysisResultsByQuestion.get(questionId);
                        tempResults.add(ar);
                    } else {
                        List<DbAchillesResult> tempResults = new ArrayList<>();
                        tempResults.add(ar);
